...
module.exports = function(dialect) {
	...
	dialect.blocks.add('field', function(params) {
		var select = params.select || params.$select;
		...
		if (select) {
			result = this.buildBlock('select', {select: select});
		} else if (query) {
			result = this.buildBlock('query', {query: query});
		} else if (field) {
			result = this.buildBlock('name', {name: field});

			var table = params.table || params.$table;
			if (table) {
				result = this.buildBlock('table', {table: table}) + '.' + result;
			}

			if (expression) {
				if (!_.isArray(expression)) expression = [expression];
				var exprSuffix = (new Array(expression.length + 1)).join(')');
				result = expression.join('(') + '(' + result + exprSuffix;
			}
		} else if (expression) {
			result = this.buildBlock('expression', {expression: expression});
		} else {
			throw new Error('Neither `select`, `query`, `field`, `expression` ' +
		...
		if (alias) {
			result += ' ' + this.buildBlock('alias', {alias: alias});
		}
				...
				if (_.isArray(fields)) {
					result = _(fields).map(function(item) {
						if (_.isObject(item) && !_.isEmpty(item)) {
							...
							if (item.select || item.query || item.field || item.expression) {
	...
	dialect.blocks.add('table', function(params) {
		return this.buildBlock('name', {name: params.table});
	});
	...
	function buildWith(builder, propName, params) {
		var withList = params[propName];
		var result = '';

		// if with clause is array -> make each withItem
		if (_.isArray(withList)) {
			result = _(withList).map(function(withItem) {
				return builder.buildTemplate('withItem', withItem);
			}).join(', ');
	...
	dialect.blocks.add('with', function(params) {
		return 'with ' + buildWith(this, 'with', params);
	});

	dialect.blocks.add('withRecursive', function(params) {
		return 'with recursive ' + buildWith(this, 'withRecursive', params);
	});
