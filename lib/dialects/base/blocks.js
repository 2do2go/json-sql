...
var fieldProps = ['select', 'query', 'field', 'value', 'name', 'func', 'expression'];
var isFieldObject = function(obj) {
	return _.isObject(obj) && !_.isArray(obj) && hasSome(obj, fieldProps);
};
...
module.exports = function(dialect) {
	...
	dialect.blocks.add('field', function(params) {
		...
		if (isSimpleValue(field) || _.isArray(field)) {
			field = _.object([_.isString(field) ? type : 'value'], [field]);
		}

		if (!isFieldObject(field)) {
			throw new Error('Neither `' + fieldProps.join('`, `') + '` properties are not set');
		}

		var result = '';

		if (_.has(field, 'select')) {
			result = dialect.buildBlock('select', {select: field.select});
		} else if (_.has(field, 'query')) {
			result = dialect.buildBlock('query', {query: field.query});
		} else if (_.has(field, 'field')) {
			result = dialect.buildBlock('field', {field: field.field, type: 'name'});
		} else if (_.has(field, 'value')) {
			result = dialect.buildBlock('value', {value: field.value});
		} else if (_.has(field, 'name')) {
			result = dialect.buildBlock('name', {name: field.name});

			if (_.has(field, 'table')) {
				result = dialect.buildBlock('table', {table: field.table}) + '.' + result;
			}
		} else if (_.has(field, 'func')) {
			result = dialect.buildBlock('func', {func: field.func});
		} else if (_.has(field, 'expression')) {
			result = dialect.buildBlock('expression', {expression: field.expression});
		}

		if (_.has(field, 'cast')) {
			result = 'cast(' + result + ' as ' + field.cast + ')';
		}

		if (_.has(field, 'alias')) {
			result += ' ' + dialect.buildBlock('alias', {alias: field.alias});
		}
	...
	dialect.blocks.add('fields', function(params) {
		...
		if (_.isArray(fields)) {
			fields = _(fields).map(function(field) {
				if (isSimpleValue(field) || isFieldObject(field)) {
					// if field has simple type or is field object: {name: '', table: 't', alias: 'r'}
					return dialect.buildBlock('field', {field: field, type: 'name'});
				} else {
	...
	dialect.blocks.add('func', function(params) {
		...
		if (_.isArray(func.args)) {
			args = _(func.args).map(function(arg) {
				return dialect.buildBlock('field', {field: arg, type: 'value'});
			}).join(', ');
		}
	...
	dialect.blocks.add('expression', function(params) {
		...
		return expression.pattern.replace(/\{([a-z0-9]+)\}/ig, function(fullMatch, block) {
			...
			return dialect.buildBlock('field', {field: values[block], type: 'value'});
		}).trim();
	});
	...
	dialect.blocks.add('values', function(params) {
		...
		return dialect.buildTemplate('insertValues', {
			fields: fields,
			fieldValues: _(values).map(function(row) {
	...
	dialect.blocks.add('fieldValues', function(params) {
		return _(params.fieldValues).map(function(row) {
			return '(' + row.join(', ') + ')';
		}).join(', ');
	});
