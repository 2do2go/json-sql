...
module.exports = function(dialect) {
	...
	dialect.blocks.add('field', function(params) {
		...
		if (!field && !expression) {
			throw new Error('Neither field name nor expression aren\'t set.');
		}
		...
		if (field) {
			...
			if (expression) {
				...
				var exprSuffix = _(expression).map(function() {
					return ')';
				}).join('');
	...
	dialect.blocks.add('table', function(params) {
		...
		table = this._wrapIdentifier(table);
		return table;
	});
	...
	dialect.blocks.add('name', function(params) {
		...
		return this._wrapIdentifier(name);
	});
	...
	dialect.blocks.add('alias', function(params) {
		...
		return 'as ' + this._wrapIdentifier(alias);
	});
	...
	dialect.blocks.add('top', function(params) {
		return 'top ' + this._pushValue(params.top);
	});
	...
	dialect.blocks.add('output', function(params) {
		return 'output ' + this.buildBlock('fields', {fields: params.output});
	});
