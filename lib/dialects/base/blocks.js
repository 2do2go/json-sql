...
module.exports = function(dialect) {
	...
	dialect.blocks.add('value', function(params) {
		...
		this.pushValue(placeholder, params.value);
		placeholder = this.wrapPlaceholder(placeholder);
		return placeholder;
	});
	...
	dialect.blocks.add('limit', function(params) {
		return 'limit ' + this.buildBlock('value', {value: params.limit});
	});
	...
	dialect.blocks.add('offset', function(params) {
		...
		return limit + 'offset ' + this.buildBlock('value', {value: params.offset});
	});
		...
		valuesList = _(valuesList).map(function(values) {
			return _(valuesNames).map(function(field) {
				if (typeof values[field] === 'undefined' || values[field] === null) {
					...
					return self.buildBlock('value', {value: values[field]});
				}
			});
		});
