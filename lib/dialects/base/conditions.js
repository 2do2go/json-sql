...
function buildCompareCondition(builder, field, operator, value) {
	...
	if (value && _.isObject(value)) {
		...
		placeholder = builder.buildBlock('value', {value: value});
	}
...
function buildContainsCondition(builder, field, operator, value) {
	...
	var placeholders = _(value).map(function(item) {
		return builder.buildBlock('value', {value: item});
	});

	return [field, operator, '(' + placeholders.join(', ') + ')'].join(' ');
}
...
module.exports = function(dialect) {
	...
	dialect.conditions.add('$between', function(field, operator, value) {
		...
		if (value.length < 2) {
			throw new Error('"between" array length should be 2 or great.');
		}

		var placeholder1 = this.buildBlock('value', {value: value[0]});
		var placeholder2 = this.buildBlock('value', {value: value[1]});

		return [field, 'between', placeholder1, 'and', placeholder2].join(' ');
	});
};
