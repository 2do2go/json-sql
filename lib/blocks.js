...
Blocks.prototype.complexCondition = function(params) {
	...
	if (_.isObject(condition) && !_.isEmpty(condition)) {
		...
		result = '(' + _(result).chain().flatten(true).compact().value()
			.join(' ' + this._conditions.combine[operator] + ' ') + ')';
	}
...
Blocks.prototype.join = function(params) {
	...
	if (!params.table) {
		throw new this.BuilderError('Join table name is not set in query properties');
	}

	return this.buildTemplate(this._templates.block.join, params);
};

Blocks.prototype.joins = function(params) {
	var self = this;
	...
	if (_.isArray(joins)) {
		result = _(joins).map(function(join) {
			return self.makeBlock('join', join);
		}).join(' ');
	// if joins is object -> set table name from key and make each join
	} if (_.isObject(joins)) {
		result = _(joins).map(function(join, table) {
			if (!join.table) {
				join.table = table;
			}
...
Blocks.prototype.values = function(params) {
	...
	var fields = _(valuesList).chain().map(function(value) {
		return _(value).keys();
	}).flatten().uniq().value();
